    ...
    <section id="catalogo" className="container py-12 md:py-16">
      ...
      <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {products.map((p) => (
          <article
            key={p.id}
            className="group overflow-hidden rounded-lg border border-neutral-200 bg-white cv-auto"
          >
            ...
            <div className="p-3">
              <h3 className="font-medium text-neutral-900 text-sm md:text-base">{p.name}</h3>
              <p className="mt-1 text-xs md:text-sm text-neutral-600 h-8 md:h-10 overflow-hidden">
                {p.description}
              </p>
              <div className="mt-2 flex items-center justify-between">
                <span className="inline-flex items-center rounded-full bg-mustard/20 px-2 py-0.5 text-[10px] md:text-xs font-medium text-wood">
                  Disponible
                </span>
                <button
                  onClick={() => onWhatsApp?.(p)}
                  className="inline-flex items-center gap-2 rounded-md bg-brick px-2.5 py-1.5 text-xs md:text-sm text-white hover:bg-brick-700"
                >
                  <svg
